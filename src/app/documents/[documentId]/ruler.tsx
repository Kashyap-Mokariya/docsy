...
const Marker = ({ position, isLeft, isDragging, onDoubleClick, onMouseDown }: MarkerProps) => {
    ...
    return (
        ...
        >
            ...
            <div
                className='absolute left-1/2 top-4 transform -translate-x-1/2 scale-x-[0.5] h-[140vh] w-[1px] bg-[#3B72F6]'
                style={
                    {
...
export const Ruler = (props: Props) => {
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        ...
        const MIN_SPACE_BEWTWEEN_MARKERS = 100
        ...
        if ((isDraggingLeft || isDraggingRight) && rulerRef.current) {
            ...
            if (container) {
                ...
                if (isDraggingLeft) {
                    const maxLeftPosition = PAGE_WIDTH - rightMargin - MIN_SPACE_BEWTWEEN_MARKERS
                    const newLeftPosition = Math.min(rawPosition, maxLeftPosition)
                    setLeftMargin(newLeftPosition)
                }
                else if (isDraggingRight) {
                    const maxRightPosition = PAGE_WIDTH - (leftMargin + MIN_SPACE_BEWTWEEN_MARKERS)
